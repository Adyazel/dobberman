import React, { useRef, useState } from 'react';
import './Login.css';
import { crud } from '../../conexion/conexion.js';
import { useNavigate } from 'react-router-dom';



export const Login = () => {
  const [bild, setbild] = useState(false);
  const inputcorreoF = useRef("a");
  const [pregunta, setPregunta] = useState("");
  const inputcorreo = useRef("");
  const inputres = useRef("");
  const inputpass = useRef("");
  const navigate = useNavigate();
  const [newPass, setNewPass] = useState("");
  const getLogin = async (event) => {
    event.preventDefault();
    const response = await crud.getUser(inputcorreo.current.value, inputpass.current.value);
  
    if (response.success) {
      localStorage.setItem("nombreus", response.user.nombre); // Guarda el nombre de usuario en el localStorage
      localStorage.setItem("role", response.user.role); // Guarda el rol de usuario en el localStorage
      localStorage.setItem("correo", response.user.correo); // Guarda el correo de usuario en el localStorage
      localStorage.setItem("pass", response.user.pass);
      localStorage.setItem("estado",response.user.estado);
      localStorage.setItem("skills",response.user.skills); // Guarda la contraseña de usuario en el localStorage
      navigate('/home'); // Redirige a la página de inicio después del login exitoso
      window.location.reload();
    } else {
      alert(response.error); // Muestra un mensaje de error
    }
  };


  const getPregun = async () => {
    if (inputcorreoF.current.value !== "") {
      const response = await crud.getPregunta(inputcorreoF.current.value);
      if (response) {
        setPregunta(response);
      } else {
        console.log("No se encontró la pregunta");
      }
    } else {
      console.log("Correo está vacío");
    }
  };



  const getRespuesta = async (event) => {
    event.preventDefault();
    const response = await crud.getRes(inputcorreoF.current.value, inputres.current.value);
  
    if (response === inputres.current.value) {

      


     const response2 = await crud.updateUser(inputcorreoF.current.value, prompt("Ingrese su nueva contraseña:"));

      if(response2){
        alert("Contraseña actualizada correctamente");
      }
      // Redirige a la página de inicio después del login exitoso
      window.location.reload();
    } else {
      alert(response.error); // Muestra un mensaje de error
    }
  };


  return (
    <>
    {!bild ? 
    <div className='Login'>
      <div>
        <div className='fondologin'><span></span></div>
        <h1>LOG IN</h1>
        <form onSubmit={getLogin}>
          <label htmlFor='correoL'>Email:</label>
          <input type='email' id='correoL' ref={inputcorreo} name='correoL'  />

          <label htmlFor='pass1'>Contraseña:</label>
          <input type='password' id='pass1' ref={inputpass} name='pass1'/>
          <p className='btn_forgot' onClick={() => {setbild(!bild)}} >Forgot Password?</p>
          <button type="submit">LOG IN</button>
        </form>
        <div className='foot_btn'>
          <p></p>
          <button onClick={()=>{navigate("/Acount");}}>+</button>
        </div>      
      </div>
    </div>
    :
    <div className='Login'>
      <div>
        <div className='fondologin'><span></span></div>
        <h1>RECUPERAR</h1>
        <form onSubmit={getRespuesta}>
          <label htmlFor='correoF'>Email:</label>
          <input type='email' id='correoF' ref={inputcorreoF} name='correoF' onBlur={getPregun}/>

          

          <label htmlFor='pregunta'>{pregunta || 'Pregunta de seguridad'}</label>
          <input type='text' id='pregunta' ref={inputres} name='pregunta'/>
          <p className='btn_forgot' onClick={() => {setbild(!bild)}} >Iniciar Sesion</p>
          <button type="submit">Recuperar</button>
        </form>
        <div className='foot_btn'>
          <pn></pn>
          {/* <button>+</button> */}
        </div>      
      </div>
    </div>
    }
    </>

  )
}
export default Login;